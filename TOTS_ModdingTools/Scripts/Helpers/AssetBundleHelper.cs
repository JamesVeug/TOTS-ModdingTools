...
namespace TOTS_ModdingTools.Helpers
{
    public static class AssetBundleHelper
    {
        public static AssetBundle LoadAssetBundle(string filename, Assembly target)
        {
            ...
            if (string.IsNullOrEmpty(resourceName))
            {
                ...
                throw new KeyNotFoundException(
                    $"Could not find resource matching {filename} in assembly {target}. Resources: {allResources}");
            }
            ...
            using (Stream resourceStream = target.GetManifestResourceStream(resourceName))
            {
                using (MemoryStream memStream = new MemoryStream())
                {
                    ...
                    if (bundle == null)
                    {
                        APILogger.LogError($"Tried getting asset bundle from bytes but failed! Is the path wrong?");
                        return null;
                    }
        ...
        public static bool TryLoadAssetBundleFromFile(string pathToAssetBundle, out AssetBundle AssetBundle)
        {
            ...
            if (!Path.IsPathRooted(pathToAssetBundle))
            {
                ...
                if (files.Length < 1)
                    throw new FileNotFoundException(
                        $"Could not find relative artwork file!\nFile name: {pathToAssetBundle}\n" +
            ...
            if (AssetBundle == null)
            {
                APILogger.LogError($"Tried getting asset bundle at path: '{fullPath}' but failed! Is the path wrong?");
                return false;
            }
        ...
        public static bool TryGet<T>(AssetBundle bundle, string prefabName, out T prefab) where T : Object
        {
            if (bundle == null)
            {
                APILogger.LogError($"Tried getting prefab from {prefabName} but the assetbundle is null!");
                prefab = default(T);
                return false;
            }
            ...
            {
                APILogger.LogError(
                    $"Tried getting prefab '{prefabName}' from asset bundle but failed! Is the prefab name or type wrong?");
                return false;
            }
        ...
        public static bool TryGet<T>(string pathToAssetBundle, string prefabName, out T prefab) where T : Object
        {
            ...
            {
                APILogger.LogError(
                    $"Tried getting asset bundle at path: '{pathToAssetBundle}' but failed! Is the path wrong?");
                prefab = default(T);
                return false;
            }
            ...
            {
                APILogger.LogError(
                    $"Tried getting prefab '{prefabName}' from asset bundle at path: '{pathToAssetBundle}' but failed! Is the prefab name or type wrong?");
                return false;
            }
        ...
        public static bool TryGet<T>(byte[] resources, string prefabName, out T prefab) where T : Object
        {
            ...
            {
                APILogger.LogError(
                    $"Tried getting prefab '{prefabName}' from asset bundle from bytes' but failed! Is the prefab name or type wrong?");
                return false;
            }
