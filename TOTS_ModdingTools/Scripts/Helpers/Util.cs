...
namespace TOTS_ModdingTools.Helpers
{
    public static class Util
    {
        ...
        public static List<FieldInfo> AllFields(Type type, string[] fieldNames)
        {
            ...
            fields.AddRange(fieldInfos.Where(a =>
                Array.IndexOf(fieldNames, a.Name) >= 0 && !fields.Exists(b => b.Name == a.Name)));
        ...
        public static List<PropertyInfo> AllProperties(Type type, string[] propertyNames)
        {
            ...
            properties.AddRange(propertyInfos.Where(a =>
                Array.IndexOf(propertyNames, a.Name) >= 0 && !properties.Exists(b => b.Name == a.Name)));
        ...
        public static T[] SafeGetComponentsInChildren<T>(this Component go, bool includeInactive = false)
            where T : Component
        {
        ...
        public static T SafeGetComponentInChildren<T>(this GameObject go, bool includeInactive = false)
            where T : Component
        {
            T componentInChildren = go.GetComponentInChildren<T>(includeInactive);
            if (componentInChildren != null)
        ...
        public static T[] SafeGetComponentsInChildren<T>(this GameObject go, bool includeInactive = false)
            where T : Component
        {
            T[] componentInChildren = go.GetComponentsInChildren<T>(includeInactive);
            if (componentInChildren != null)
            ...
            APILogger.LogError($"Could not find {typeof(T).FullName} components in children of {go.FullName()}!");
            return componentInChildren;
        }
        ...
        public static bool TryFindChildRecursive<T>(this GameObject go, string name, out T found,
            bool errorIfNotFound = true) where T : Component
        {
            found = default;

            Transform transform = FindRecursiveInternal(go.transform, name);
            if (transform != null)
            {
            ...
            {
                APILogger.LogError(
                    $"Could not find child {name} in {go.FullName()} with component {typeof(T).FullName}!");
            }

            return false;
        }

        public static bool TryFindChildRecursive<T>(this Component t, string name, out T found,
            bool errorIfNotFound = true) where T : Component
        {
            found = default;
            ...
            {
                APILogger.LogError(
                    $"Could not find child {name} in {t.FullName()} with component {typeof(T).FullName}!");
            }

            return false;
        }

        public static bool TryFindChild(this Component t, string name, out GameObject found,
        ...
        public static void CompareUnityVersions(string major, string minor, string patch)
        {
            ...
            {
                APILogger.LogError(
                    $"The major Unity version has changed from {expectedVersion} to {currentVersion}! This will likely have code breaking changes and also require rebuilding asset bundles with the new unity version.");
            }
            else if (currentMinor != minor)
            {
                APILogger.LogError(
                    $"The minor Unity version has changed from {expectedVersion} to {currentVersion}! This will likely have code breaking changes and also require rebuilding asset bundles with the new unity version.");
            }
            else if (currentPatch != patch)
            {
                APILogger.LogWarning(
                    $"The patch Unity version has changed from {expectedVersion} to {currentVersion}! This may have code breaking changes!");
            }
            else
            {
        ...
        public static RectTransform GetRectTransform(this Component component)
        {
            ...
            else if (component.transform is RectTransform rectTransform2)
            {
                return rectTransform2;
            }

            APILogger.LogError($"Could not find RectTransform in {component.FullName()}!");
            return null;
        }
    }
}