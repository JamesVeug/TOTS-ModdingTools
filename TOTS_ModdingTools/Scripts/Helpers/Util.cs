...
public static class Util
{
    ...
    public static T[] SafeGetComponentsInChildren<T>(this GameObject go, bool includeInactive=false) where T : Component
    {
        T[] componentInChildren = go.GetComponentsInChildren<T>(includeInactive);
        if(componentInChildren != null)
        {
    ...
    public static bool TryFindChildRecursive<T>(this GameObject go, string name, out T found, bool errorIfNotFound=true) where T : Component
    {
        found = default;
        ...
        if (errorIfNotFound)
        {
            APILogger.LogError($"Could not find child {name} in {go.FullName()} with component {typeof(T).FullName}!");
        }
