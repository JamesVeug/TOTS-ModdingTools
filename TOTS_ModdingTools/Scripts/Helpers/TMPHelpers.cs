    ...
    public static class TMPHelpers
    {
        ...
        private static Texture2D CombineTexturesIntoPower2(List<Texture2D> textures, out List<TMP_Sprite> sprites)
        {
            ...
            for (var i = 0; i < sortedTextures.Count; i++)
            {
                ...
                combinedTexture.SetPixels(shelfWidth, totalHeight, texture2D.width, texture2D.height,
                    texture2D.GetPixels());
                Sprite sprite1 = Sprite.Create(combinedTexture, new Rect(0, 0, width, height), Vector2.zero, 100, 0,
