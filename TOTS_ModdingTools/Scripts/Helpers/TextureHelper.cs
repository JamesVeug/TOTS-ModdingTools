...
namespace TOTS_ModdingTools.Helpers
{
    ...
    public static class TextureHelper
    {
        ...
        public enum SpriteType
        {
            ...
            DecorationTierIcon = 11,
        }

        private static Vector2 DEFAULT_PIVOT = new Vector2(0.5f, 0.5f);

        private static readonly Dictionary<SpriteType, Rect> SPRITE_RECTS = new Dictionary<SpriteType, Rect>()
        {
            ...
            { SpriteType.DecorationTierIcon, new Rect(0.0f, 0.0f, 128.0f, 128.0f) },
        };

        private static readonly Dictionary<SpriteType, Vector2> SPRITE_PIVOTS = new Dictionary<SpriteType, Vector2>()
        {
            ...
            { SpriteType.DecorationTierIcon, DEFAULT_PIVOT },
        };
        ...
        public static byte[] ReadArtworkFileAsBytes(string pathCardArt)
        {
            ...
            if (!File.Exists(pathCardArt))
                throw new FileNotFoundException(
                    $"Absolute path to artwork file does not exist!\nFile name: {pathCardArt}", pathCardArt);
        ...
        public static Texture2D GetImageAsTexture(string pathCardArt, Assembly target,
        ...
        public static Sprite ConvertTexture(this Texture2D texture, SpriteType spriteType,
            FilterMode filterMode = FilterMode.Point)
        {
            texture.filterMode = filterMode;
        ...
        public static Sprite GetImageAsSprite(string pathCardArt, SpriteType spriteType,
            FilterMode filterMode = FilterMode.Point)
        {
        ...
        public static Sprite GetImageAsSprite(string pathCardArt, Assembly target, SpriteType spriteType,
            FilterMode filterMode = FilterMode.Point)
        {
        ...
        private static void VerifyTexture(Texture2D texture, SpriteType spriteType)
        {
            ...
            if (texture.width != rect.width || texture.height != rect.height)
            {
                APILogger.LogWarning(
                    $"Texture dimensions for {texture.name} should be {rect.width}x{rect.height} but instead it is {texture.width}x{texture.height}.");
            }
        }
        ...
        public static byte[] GetResourceBytes(string filename, Assembly target)
        {
            ...
            string resourceName = target.GetManifestResourceNames()
                .FirstOrDefault(r => r.ToLowerInvariant().EndsWith(lowerKey));
        ...
        public static Texture2D CombineTextures(List<Texture2D> pieces, Texture2D baseTexture, int xStep = 0,
            int yStep = 0, int xOffset = 0, int yOffset = 0)
        {
            if (pieces != null)
            {
                for (int j = 0; j < pieces.Count; j++)
                    if (pieces[j] != null)
                        baseTexture.SetPixels(xOffset + xStep * j, yOffset + yStep * (pieces.Count - j - 1),
                            pieces[j].width, pieces[j].height, pieces[j].GetPixels(), 0);
        ...
        public static Texture2D GetReadableTexture(Texture2D texture)
        {
            ...
            if (!texture.isReadable)
            {
                ...
                Texture2D readableText =
                    new Texture2D(texture.width, texture.height, TextureFormat.RGBA32, false, true);
